    ...
    describe('update', () => {
        describe('wild patches', () => {
            it('return latestVersion when there is nothing to replace', () => {
                ...
                expect(getNewSemver({ version, latestVersion })).toEqual({ newVersion: '1.1.2', semVerChange: PATCH });
            });
            ...
            it('return latestVersion with a patch wild and minor update', () => {
                ...
                expect(getNewSemver({ version, latestVersion })).toEqual({ newVersion: '1.2.x', semVerChange: MINOR });
            });
            ...
            it('return latestVersion with a patch tilda and minor update', () => {
                const version = '~1.1.1';
                const latestVersion = '1.2.2';
                expect(getNewSemver({ version, latestVersion })).toEqual({ newVersion: '~1.2.2', semVerChange: MINOR });
            });
        });
