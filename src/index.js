...
const config = configPathJson ? JSON.parse(fs.readFileSync(configPathJson)) : require(configPathJs) || {};
...
FileHound.create()
    ...
    .then((paths) => {
        ...
        const types = [argv.deps && 'dependencies', argv.dev && 'devDependencies', argv.peer && 'peerDependencies']
            .filter(Boolean)
            .join(', ');
        const owners = [argv.internal && 'internal scopes', argv.external && 'external scopes']
            .filter(Boolean)
            .join(', ');
        log(`|`);
